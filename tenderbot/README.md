...
WEB_HOST=0.0.0.0
WEB_PORT=8000
# Для кнопки Mini App в Telegram (локально — ngrok URL): MINIAPP_BASE_URL=https://xxxx.ngrok-free.app
```

### 3. Запуск

**Локально с ngrok (кнопка «Открыть приложение» в боте):**
1. В одном терминале: `ngrok http 8000`
2. В `.env` укажите: `MINIAPP_BASE_URL=https://xxxx.ngrok-free.app` (URL из вывода ngrok)
3. В корне проекта: `python run.py` — запустятся бот и веб, админка и Mini App доступны по ngrok-ссылке
