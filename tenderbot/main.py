...
async def main() -> None:
    ...
    # Запуск фонового планировщика тендеров (автозакрытие + напоминания)
    from services.tender_scheduler import tender_scheduler_loop
    asyncio.create_task(tender_scheduler_loop(bot))
    logger.info("Tender scheduler task started.")
