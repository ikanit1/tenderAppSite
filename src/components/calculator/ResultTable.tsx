...
interface ResultTableProps {
  ...
  /** Скрыть группы оборудования (только итоги + рассрочка) */
  hideGroups?: boolean;
  /** Количество квартир в доме (для расчёта "с квартиры") */
  flatCount?: number;
  onFlatsChange?: (val: number) => void;
  /** Дополнительная сумма (напр. домофония) для строки общего итога */
  extraTotal?: number;
  extraLabel?: string;
  /** Callback при изменении параметров рассрочки */
  onInstallmentChange?: (params: { selectedMonths: number; initialPayment: number }) => void;
}
...
export function ResultTable({
  ...
  const handleSelectMonths = (months: number) => {
    ...
    onInstallmentChange?.({ selectedMonths: months, initialPayment: Math.max(0, parseInt(initialPaymentText || '0', 10) || 0) });
  };

  const handleInitialPaymentChange = (text: string) => {
    setInitialPaymentText(text);
    onInstallmentChange?.({ selectedMonths, initialPayment: Math.max(0, parseInt(text || '0', 10) || 0) });
  };

  const projectTotal = result.grandTotal + extraTotal;
  // flatCount сюда приходит уже как effectiveFlats (с учётом домофонии)
  ...
  return (
    <div className={styles.wrap}>
      ...
      {!hideGroups && result.groups.map((group) => (
      ...
      {!hideGroups && <motion.div className={styles.totalBlock} variants={cardVariants} initial="visible" animate="visible">
        ...
        <div className={styles.totalRowHighlight}>
          <span>ИТОГО видеонаблюдение</span>
          <strong>{formatKzt(result.grandTotal)}</strong>
        </div>
        {extraTotal > 0 && (<>
          <div className={styles.totalRow} style={{ marginTop: '0.5rem' }}>
            <span>{extraLabel ?? 'Домофония'}</span>
            <strong>{formatKzt(extraTotal)}</strong>
          </div>
          <div className={`${styles.totalRowHighlight} ${styles.totalRowGrand}`}>
            <span>ИТОГО ПО ПРОЕКТУ</span>
            <strong>{formatKzt(result.grandTotal + extraTotal)}</strong>
          </div>
        </>)}
        {extraTotal === 0 && (
          <div className={`${styles.totalRowHighlight} ${styles.totalRowGrand}`}>
            ...
            <div className={styles.installmentInputRow}>
              <label className={styles.installmentField}>
                ...
                <input
                  ...
                  placeholder={flatCount > 0 ? String(flatCount) : '0'}
                />
              </label>
            </div>
          )}
              ...
              <tbody>
                {paymentsByMonths.map((p) => (
                  <tr
                    ...
                    tabIndex={0}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') handleSelectMonths(p.months);
                    }}
